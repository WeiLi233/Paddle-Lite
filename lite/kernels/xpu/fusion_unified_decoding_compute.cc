...
void FusionUnifiedDecodingCompute::PrepareForRun() {
  ...
  word_embedding_ = xft::xftMat<float>(const_cast<float*>(param.word_embedding_->data<float>()), 
                                      {param.word_embedding_->dims()[0], param.word_embedding_->dims()[1]});
  positional_embedding_ = xft::xftMat<float>(const_cast<float*>(param.positional_embedding_weight_->data<float>()), 
                                      {param.positional_embedding_weight_->dims()[0], param.positional_embedding_weight_->dims()[1]});
  type_embedding_ = xft::xftMat<float>(const_cast<float*>(param.type_embedding_weight_->data<float>()), 
                                      {param.type_embedding_weight_->dims()[0], param.type_embedding_weight_->dims()[1]});
  ...
  // self_k_xx_ and self_v_xx_ are useless
  ...
  for(int32_t layer=0; layer<fud_param_.num_layer; layer++){
    self_ln_weight_.emplace_back(const_cast<float*>(param.self_ln_weight_[layer]->data<float>()), 
                                  xft::xftVec<float>::dim_t{param.self_ln_weight_[layer]->dims()[0]});
    self_ln_bias_.emplace_back(const_cast<float*>(param.self_ln_bias_[layer]->data<float>()), 
                                  xft::xftVec<float>::dim_t{param.self_ln_bias_[layer]->dims()[0]});
    self_q_weight_.push_back(xft::quant_weight_from_host<float, int16_t>(ctx,
                      const_cast<float*>(param.self_q_weight_[layer]->data<float>()),
                      {param.self_q_weight_[layer]->dims()[0], param.self_q_weight_[layer]->dims()[1]}, true));
    self_q_bias_.emplace_back(const_cast<float*>(param.self_q_bias_[layer]->data<float>()), 
                                  xft::xftVec<float>::dim_t{param.self_q_bias_[layer]->dims()[0]});
    self_out_weight_.push_back(xft::quant_weight_from_host<float, int16_t>(ctx,
                      const_cast<float*>(param.self_out_weight_[layer]->data<float>()),
                      {param.self_out_weight_[layer]->dims()[0], param.self_out_weight_[layer]->dims()[1]}, true));
    self_out_bias_.emplace_back(const_cast<float*>(param.self_out_bias_[layer]->data<float>()), 
                                  xft::xftVec<float>::dim_t{param.self_out_bias_[layer]->dims()[0]});
    ffn_ln_weight_.emplace_back(const_cast<float*>(param.ffn_ln_weight_[layer]->data<float>()), 
                                  xft::xftVec<float>::dim_t{param.ffn_ln_weight_[layer]->dims()[0]});
    ffn_ln_bias_.emplace_back(const_cast<float*>(param.ffn_ln_bias_[layer]->data<float>()), 
                                  xft::xftVec<float>::dim_t{param.ffn_ln_bias_[layer]->dims()[0]});
    ffn_inter_weight_.push_back(xft::quant_weight_from_host<float, int16_t>(ctx,
                      const_cast<float*>(param.ffn_inter_weight_[layer]->data<float>()),
                      {param.ffn_inter_weight_[layer]->dims()[0], param.ffn_inter_weight_[layer]->dims()[1]}, true));
    ffn_inter_bias_.emplace_back(const_cast<float*>(param.ffn_inter_bias_[layer]->data<float>()), 
                                  xft::xftVec<float>::dim_t{param.ffn_inter_bias_[layer]->dims()[0]});
    ffn_out_weight_.push_back(xft::quant_weight_from_host<float, int16_t>(ctx,
                      const_cast<float*>(param.ffn_out_weight_[layer]->data<float>()),
                      {param.ffn_out_weight_[layer]->dims()[0], param.ffn_out_weight_[layer]->dims()[1]}, true));
    ffn_out_bias_.emplace_back(const_cast<float*>(param.ffn_out_bias_[layer]->data<float>()), 
                                  xft::xftVec<float>::dim_t{param.ffn_out_bias_[layer]->dims()[0]});
  }
  decoder_ln_weight_ = xft::xftVec<float>(const_cast<float*>(param.decoder_ln_weight_->data<float>()), 
                                          {param.decoder_ln_weight_->dims()[0]});
  decoder_ln_bias_ = xft::xftVec<float>(const_cast<float*>(param.decoder_ln_bias_->data<float>()), 
                                          {param.decoder_ln_bias_->dims()[0]});
  trans_weight_ = xft::quant_weight_from_host<float, int16_t>(ctx,
                    const_cast<float*>(param.trans_weight_->data<float>()),
                    {param.trans_weight_->dims()[0], param.trans_weight_->dims()[1]}, true);
  trans_bias_ = xft::xftVec<float>(const_cast<float*>(param.trans_bias_->data<float>()), 
                                          {param.trans_bias_->dims()[0]});
  lm_ln_weight_ = xft::xftVec<float>(const_cast<float*>(param.lm_ln_weight_->data<float>()), 
                                          {param.lm_ln_weight_->dims()[0]});
  lm_ln_bias_ = xft::xftVec<float>(const_cast<float*>(param.lm_ln_bias_->data<float>()), 
                                          {param.lm_ln_bias_->dims()[0]});
  emb_weight_ = xft::quant_weight_from_host<float, int16_t>(ctx,
                    const_cast<float*>(param.embedding_weight_->data<float>()),
                    {param.embedding_weight_->dims()[0], param.embedding_weight_->dims()[1]}, true);
  emb_bias_ = xft::xftVec<float>(const_cast<float*>(param.embedding_bias_->data<float>()), 
...
void FusionUnifiedDecodingCompute::Run() {
  ...
  // input
  xft::xftMat<int32_t> input_ids(const_cast<int32_t*>(param.input_ids_->data<int32_t>()),
                              {param.input_ids_->dims()[0], param.input_ids_->dims()[1]});
  xft::xftVec<int32_t> mem_seq_len(const_cast<int32_t*>(param.mem_seq_len_->data<int32_t>()),
                              {param.mem_seq_len_->dims()[0]});
  xft::xftTensor<float, 4> attn_mask(const_cast<float*>(param.attn_mask_->data<float>()),
                              {param.attn_mask_->dims()[0], param.attn_mask_->dims()[1], param.attn_mask_->dims()[2], param.attn_mask_->dims()[3]});
  xft::xftMat<int32_t> pos_ids(const_cast<int32_t*>(param.position_ids_->data<int32_t>()),
                              {param.position_ids_->dims()[0], param.position_ids_->dims()[1]});
  xft::xftMat<int32_t> type_ids(const_cast<int32_t*>(param.type_id_->data<int32_t>()),
                              {param.type_id_->dims()[0], param.type_id_->dims()[1]});
  xft::xftMat<int32_t> dec_pos_ids(const_cast<int32_t*>(param.decoder_position_ids_->data<int32_t>()),
                              {param.decoder_position_ids_->dims()[0], param.decoder_position_ids_->dims()[1]});
  xft::xftMat<int32_t> dec_type_ids(const_cast<int32_t*>(param.decoder_type_id_->data<int32_t>()),
                              {param.decoder_type_id_->dims()[0], param.decoder_type_id_->dims()[1]});
  // output
  xft::xftMat<int32_t> output_ids(param.output_ids_->mutable_data<int32_t>(TARGET(kXPU)), {max_out_len, batch_size});
  xft::xftVec<int32_t> output_seq_len(param.sequence_length_->mutable_data<int32_t>(TARGET(kXPU)), {batch_size});
  xft::xftVec<float> output_scores(param.output_scores_->mutable_data<float>(TARGET(kXPU)), {batch_size});
  
  int32_t ret = xft::fusion_unified_decoding<float, int16_t, int16_t>(ctx,
      input_ids, mem_seq_len, attn_mask,
      pos_ids, type_ids,
      dec_pos_ids, dec_type_ids,
      word_embedding_, positional_embedding_, type_embedding_,
      self_ln_weight_, self_ln_bias_, 
      self_q_weight_, self_q_bias_, 
      self_k_weight_, self_k_bias_, 
      self_v_weight_, self_v_bias_, 
      self_out_weight_, self_out_bias_,
      ffn_ln_weight_, ffn_ln_bias_,
      ffn_inter_weight_, ffn_inter_bias_,
      ffn_out_weight_, ffn_out_bias_,
      decoder_ln_weight_, decoder_ln_bias_,
      trans_weight_, trans_bias_,
